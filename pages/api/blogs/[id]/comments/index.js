    ...
    try {
      ...
      const newComment = await prisma.comment.create({
        data: {
          ...
          parentId: parentId ? parseInt(parentId) : null, // if the comment is a reply, link it to the parent comment
          upvote: 0,
          downvote: 0,
          hidden: false,
        },
      ...
      const comments = await prisma.comment.findMany({
        ...
        include: {
          ...
          replies: {
            ...
            orderBy: {
              createdAt: "asc", // Show replies in ascending order by time (you can change this order if preferred)
            },
          },
        },
      });
...
const handlerWithAuth = (req, res) => {
  if (req.method === "POST") {
    return jwtMiddleware(handler, ["USER", "ADMIN"])(req, res); // uses currying here: jwtMiddleware returns a function that then takes in req and res as params
  }
